...
<div class="bg py-5">
    <div class="container py-5 custom-gradient-shadow">
        <div class="d-flex flex-column justify-content-center p-5">
            <div class="fs-1 fw-bold mb-4 text-center">ศูนย์ข้อมูลข่าวสาร <br>
                <span class="fs-3">{{ $InformationCenter->name }}</span>
            </div>

            <p class="text-muted">
                วันที่เผยแพร่: {{ \Carbon\Carbon::parse($InformationCenter->created_at)->format('d-m-Y') }}
            </p>

            @if ($InformationCenterFile->count() > 0)
                <h5 class="text-secondary mt-4">ไฟล์เอกสาร</h5>
                @foreach ($InformationCenterFile as $file)
                    @if ($file->file_type == 'pdf')
                        <div class="mb-3">
                            <iframe src="{{ asset('storage/' . $file->file_path) }}" width="100%" height="700px"></iframe>
                        </div>
                    @endif
                @endforeach
            @else
