...
class AdminContactController extends Controller
{
    ...
    public function ContactUpdate(Request $request, $id)
    {
        ...
        // อัปโหลดไฟล์ Logo ถ้ามีการอัปโหลดใหม่
        if ($request->hasFile('logo_file')) {
            // ลบไฟล์เก่า
            if ($contact->logo_file) {
                Storage::disk('public')->delete($contact->logo_file);
        ...
        // อัปโหลดไฟล์ช่องทางการชำระเงิน ถ้ามีการอัปโหลดใหม่
        if ($request->hasFile('payment_file')) {
            // ลบไฟล์เก่า
            if ($contact->payment_file) {
                Storage::disk('public')->delete($contact->payment_file);
        ...
        // อัปโหลดไฟล์ช่องทางการติดต่อสอบถาม อบต. ถ้ามีการอัปโหลดใหม่
        if ($request->hasFile('contact_administration_file')) {
            // ลบไฟล์เก่า
            if ($contact->contact_administration_file) {
                Storage::disk('public')->delete($contact->contact_administration_file);
        ...
        // อัปโหลดไฟล์ช่องทางการติดต่อสอบถาม กองคลัง ถ้ามีการอัปโหลดใหม่
        if ($request->hasFile('contact_finance_file')) {
            // ลบไฟล์เก่า
            if ($contact->contact_finance_file) {
                Storage::disk('public')->delete($contact->contact_finance_file);
        ...
        $contact->update([
            'details' => $request->details,
            'logo_file' => $filePaths['logo_file'] ?? $contact->logo_file,
            'payment_file' => $filePaths['payment_file'] ?? $contact->payment_file,
            'contact_administration_file' => $filePaths['contact_administration_file'] ?? $contact->contact_administration_file,
            'contact_finance_file' => $filePaths['contact_finance_file'] ?? $contact->contact_finance_file,
            'link_map' => $request->link_map,
        ]);

        return redirect()->back()->with('success', 'ข้อมูลถูกอัปเดตเรียบร้อย');
    }
}
